...
import {
  ...
  zoomToElement as screenShareZoom, highlightElements as screenShareHighlight,
  scrollToElement as screenShareScrollTo, evalOnPage as screenShareEval,
...
== SCREEN SHARE — คู่มือการใช้งานฉลาด ==
เมื่อ admin พูดอะไรเกี่ยวกับ "แชร์จอ / ดูจอ / โชว์ / เปิดให้ดู / แสดง" บนเว็บ:

1. ถ้ายังไม่รู้ URL → ใช้ web_search หา URL ที่ถูกก่อน แล้วค่อย screen_share_start
2. เปิด URL ที่ถูกต้อง ไม่ใช่หน้า search result
3. ถ้า admin บอกให้ "ซูม / ขยาย / ดูใกล้ๆ" element ใด → screen_share_zoom(selector) หรือ screen_share_eval("document.body.style.zoom='150%'") แล้ว screen_share_snapshot
4. ถ้า admin บอก "ชี้ให้ดู / ไฮไลต์ / วงกลม" อะไร → screen_share_highlight(selectors)
5. ถ้า admin บอก "เลื่อนไปที่ / ไปดูตรงนั้น" → screen_share_scroll_to(selector)
6. ถ้า admin บอก "คลิก / กด" → screen_share_click(selector) แล้ว screen_share_snapshot
7. ถ้า admin บอก "พิมพ์ / ค้นหา" ข้อความ → screen_share_type(selector, text) แล้ว screen_share_snapshot
8. ถ้า admin บอก "ไปหน้านั้น / เปิดอีกอัน" → screen_share_navigate(url)
9. ถ้า admin บอก "หยุด / ปิด / ไม่ต้องแล้ว" → screen_share_stop

ใช้ screen_share_zoom เมื่อต้องการเน้นรายละเอียด — มันจะ crop เฉพาะ element นั้นออกมา
ใช้ screen_share_eval("document.body.style.zoom='200%'") เพื่อขยายหน้าเว็บทั้งหมด
ใช้ screen_share_eval("document.body.style.zoom='100%'") เพื่อรีเซ็ตกลับ
`;
