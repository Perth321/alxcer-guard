...
import {
  ...
  typeText as screenShareType, clickAt as screenShareClick,
  scrollPage as screenShareScroll, zoomBrowser as screenShareZoom,
  resetZoom as screenShareResetZoom, findInPage as screenShareFind,
  pressKey as screenShareKey,
...
const TOOLS = [
  ...
  // --- Screen Share (live browser view posted in the Discord channel) ---
  {
    ...
    function: {
      ...
      description: "เปิด Go Live screen share จริงๆ ในห้องเสียง Discord — ผู้ใช้ในห้องกดดูได้เลย 30fps H264. ต้องระบุ voice_channel_id (ID ของห้องเสียงที่จะ stream เข้า)",
      parameters: {
        type: "object",
        properties: {
          url: { type: "string", description: "URL ที่ต้องการเปิดใน Chrome (ควรเป็น URL จริง ไม่ใช่หน้า search)" },
          voice_channel_id: { type: "string", description: "ID ของ voice channel ที่จะ Go Live เข้า" },
        },
        required: ["url", "voice_channel_id"],
      },
    },
  },
  {
    ...
    function: {
      ...
      description: "หยุด Go Live screen share และออกจากห้องเสียง",
      parameters: { type: "object", properties: {} },
    },
  },
  {
    ...
    function: {
      ...
      description: "ระหว่าง stream อยู่: เปลี่ยน URL ไปหน้าใหม่",
      parameters: {
        type: "object",
        properties: { url: { type: "string", description: "URL ที่ต้องการไป" } },
        required: ["url"],
      },
    },
  },
  {
    ...
    function: {
      ...
      description: "ระหว่าง stream อยู่: คลิกที่พิกัด x,y บนหน้าจอ (1280x720)",
      parameters: {
        type: "object",
        properties: {
          x: { type: "number", description: "พิกัด x (0–1280)" },
          y: { type: "number", description: "พิกัด y (0–720)" },
        },
        required: ["x", "y"],
      },
    },
  },
  {
    ...
    function: {
      ...
      description: "ระหว่าง stream อยู่: พิมพ์ข้อความ (xdotool type) บนหน้าจอ",
      parameters: {
        type: "object",
        properties: { text: { type: "string", description: "ข้อความที่ต้องการพิมพ์" } },
        required: ["text"],
      },
    },
  },
  {
    ...
    function: {
      ...
      description: "ระหว่าง stream อยู่: เลื่อนหน้า up/down",
      parameters: {
        type: "object",
        properties: {
          direction: { type: "string", enum: ["up", "down"], description: "ทิศทาง" },
          steps: { type: "number", description: "จำนวนครั้ง (default 3)" },
        },
        required: ["direction"],
      },
    },
  },
  {
    type: "function",
    function: {
      name: "screen_share_zoom",
      description: "ระหว่าง stream อยู่: ซูมเบราว์เซอร์ in/out (Ctrl+/Ctrl-)",
      parameters: {
        type: "object",
        properties: {
          direction: { type: "string", enum: ["in", "out", "reset"], description: "ซูมเข้า / ออก / รีเซ็ต 100%" },
          steps: { type: "number", description: "จำนวนครั้งกด (default 2)" },
        },
        required: ["direction"],
      },
    },
  },
  {
    type: "function",
    function: {
      name: "screen_share_find",
      description: "ระหว่าง stream อยู่: ค้นหาข้อความในหน้า (Ctrl+F)",
      parameters: {
        type: "object",
        properties: { query: { type: "string", description: "ข้อความที่ต้องการค้นหา" } },
        required: ["query"],
      },
    },
  },
  {
    type: "function",
    function: {
      name: "screen_share_key",
      description: "ระหว่าง stream อยู่: กดปุ่มคีย์บอร์ด เช่น Return, Escape, ctrl+t, alt+Left",
      parameters: {
        type: "object",
        properties: { key: { type: "string", description: "xdotool key name เช่น Return, ctrl+l, alt+Left" } },
        required: ["key"],
      },
    },
  },
  {
    ...
    function: {
      ...
      description: "ดูสถานะ screen share ปัจจุบัน",
      parameters: { type: "object", properties: {} },
    },
  },
    {
  ...
  },
  {
    ...
    function: {
      name: "screen_share_zoom",
      description: "ระหว่างแชร์จอ: ซูมเข้าไปที่ element เฉพาะ (crop ภาพเฉพาะส่วนนั้น) เช่น ซูมดูตาราง, ซูมดูรูป",
      parameters: {
        type: "object",
        properties: { selector: { type: "string", description: "CSS selector ของ element ที่ต้องการซูม" } },
        required: ["selector"],
      },
    },
  },
  {
    type: "function",
    function: {
      name: "screen_share_highlight",
      description: "ระหว่างแชร์จอ: ไฮไลต์ (วงกรอบสีแดง) element หนึ่งหรือหลาย element บนหน้าจอ เพื่อชี้ให้ดู",
      parameters: {
        type: "object",
        properties: {
          selectors: { type: "array", items: { type: "string" }, description: "รายการ CSS selector" },
          color: { type: "string", description: "สีกรอบ hex เช่น #ff4444 (default) หรือ #00cc88" },
        },
        required: ["selectors"],
      },
    },
  },
  {
    type: "function",
    function: {
      name: "screen_share_scroll_to",
      description: "ระหว่างแชร์จอ: เลื่อนหน้าไปที่ element นั้น (scrollIntoView) แล้วไฮไลต์และถ่ายรูป",
      parameters: {
        type: "object",
  ...
  },
  {
    type: "function",
    function: {
      name: "screen_share_eval",
      description: "ระหว่างแชร์จอ: รัน JavaScript บนหน้าปัจจุบัน แล้วถ่ายรูปผลลัพธ์ — ใช้สำหรับซูมเองด้วย CSS scale / เปลี่ยน font size / ดึงข้อมูล",
      parameters: {
        type: "object",
        properties: { code: { type: "string", description: "JS expression ที่จะรันใน page context" } },
        required: ["code"],
      },
    },
  },
...
async function execTool(name, args, ctx) {
  ...
  switch (name) {
    ...
    case "screen_share_start": {
      const vc = args.voice_channel_id;
      if (!vc) return { error: "ต้องระบุ voice_channel_id" };
      const gId = ctx.guild?.id;
      if (!gId) return { error: "ไม่พบ guild" };
      const client = ctx.guild.client;
      const notifyCh = ctx.msg?.channel || null;
      return await startScreenShare({ url: args.url, client, guildId: gId, channelId: vc, notifyChannel: notifyCh });
    }
    ...
    case "screen_share_click":
      return await screenShareClick(args.x ?? 640, args.y ?? 360);

    case "screen_share_type":
      return await screenShareType(args.text);

    case "screen_share_scroll":
      return await screenShareScroll(args.direction || "down", args.steps || 3);

    case "screen_share_zoom":
      if (args.direction === "reset") return await screenShareResetZoom();
      return await screenShareZoom(args.direction || "in", args.steps || 2);

    case "screen_share_find":
      return await screenShareFind(args.query);

    case "screen_share_key":
      return await screenShareKey(args.key);
    ...
    case "screen_share_zoom":
      return await screenShareZoom(args.selector);

    case "screen_share_highlight":
      return await screenShareHighlight(args.selectors || [args.selector], args.color);

    case "screen_share_scroll_to":
      return await screenShareScrollTo(args.selector);

    case "screen_share_eval":
      return await screenShareEval(args.code);
...
== SCREEN SHARE — Go Live จริงๆ ใน Discord Voice ==
เมื่อ admin พูดเกี่ยวกับ "แชร์จอ / ดูจอ / โชว์ / เปิดให้ดู / Go Live":

1. ถามหรือดู voice_channel_id ก่อน (ดูจาก guild.channels หรือถาม admin)
2. ถ้าไม่รู้ URL → ใช้ web_search หา URL จริงก่อน แล้วค่อย screen_share_start
3. screen_share_start จะเปิด Chrome บน virtual display แล้ว Go Live เข้าห้องเสียง — ผู้ใช้กดดูได้จริง 30fps
4. ระหว่าง Live อยู่:
   - "ไปหน้านั้น / เปลี่ยน URL" → screen_share_navigate(url)
   - "คลิกตรงนั้น" → screen_share_click(x, y) 
   - "พิมพ์ / ค้นหา" → screen_share_type(text) หรือ screen_share_find(query)
   - "เลื่อนลง / ขึ้น" → screen_share_scroll(direction)
   - "ซูมเข้า / ออก / รีเซ็ต" → screen_share_zoom(direction)
   - "กด Enter / กด Escape" → screen_share_key("Return" / "Escape")
5. "หยุด / ปิด / ออก" → screen_share_stop

สำคัญ: screen_share_start ต้องการ voice_channel_id — ถ้า admin ไม่บอก ให้ list voice channels แล้วถามว่าจะ stream เข้าห้องไหน
